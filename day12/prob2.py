...
for row in range(len(inp)):
    for col in range(len(inp[row])):
        ...
        if inp[row][col] == 'S' or inp[row][col] == 'a':
...
# find shortest path from E to all other nodes (third arg no longer matters)
dist = shortest_path(graph, E, (0,0))
lens = []
for x in possible_starts:
    if dist[x]:
        lens.append(dist[x])

print('answer:',min(lens))
